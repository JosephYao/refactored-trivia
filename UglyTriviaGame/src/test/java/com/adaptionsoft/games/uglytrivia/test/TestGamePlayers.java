...
public class TestGamePlayers {
	...
	@Test public void player_Name_Added() {
		...
		assertTrue(gamePlayers.getAllPlayers().contains("Player1"));
		gamePlayers.add("Player2");
		assertTrue(gamePlayers.getAllPlayers().contains("Player2"));
	}
	
	@Test public void player_Status_Initialized() {
		gamePlayers.add("Player1");
		assertEquals(0, gamePlayers.getAllPlayerPlaces()[0]);
		assertEquals(0, gamePlayers.getAllPlayerPurses()[0]);
		assertFalse(gamePlayers.getAllPlayerInPenaltyBox()[0]);
	}
